...
json_path = "C:\\Users\\COLORFUL\\Downloads\\database.json"
...
def daftar_kamar():
    ...
    while True:
        ...
        if opsi == '1':
            ...
            search_kamar = input("Masukkan kamar yang ingin Anda cari: ")
...
def tambah_kamar():
    ...
    lantai = input("Masukkan lantai kamar (3, 4, atau 5): ")
    
    if lantai not in data["kamar"][lantai]:
...
def tamu(username):
        if username in data.get("Username"):
            ...
            saldo_emoney = data.get("Saldo_E Money")[index]
        print(" ╔═══════════════════════════════════════════════════════════════════════════════════╗")
        print(f"           Selamat Datang di Hotel Jatra Balikpapan, {nama} ")
        print(" ╚═══════════════════════════════════════════════════════════════════════════════════╝")
        print(f"                Saldo E-Money Anda: Rp.{saldo_emoney} ")
    ...
    try:
        ...
        if username in data["Username"]:
            ...
            if saldo_Emoney >= harga_kamar:
                ...
                invoice_path = "C:\\Pengguna\\COLORFUL\\Downloads\\invoice_reservasi_hotel.txt"
...
def topup_saldo(username):
    os.system("cls")
    print("=" * 10 + "Top Up Saldo E Money" + "=" * 10)
    try:
        topup = float(input("Masukkan jumlah top-up: Rp"))
        if topup <= 0:
            print("Jumlah top-up harus lebih besar dari 0")
            return

        konfirmasi = input(f"Apakah Anda yakin ingin mengisi saldo Rp. {topup:,}? (Benar/Tidak): ")
        if konfirmasi.lower() in ["benar", "y"]:
            penerima_index = data["Username"].index(username)
            data["Saldo_E Money"][penerima_index] += topup
            
            UpdateTable()

            print(f"Isi saldo berhasil. Saldo Anda sekarang: Rp.{data['Saldo_E Money'][penerima_index]:,.2f} ༼ つ ◕_◕ ༽つ")

            waktu = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            invoice_content = buat_invoice_isisaldo(waktu, topup, username)

            invoice_path = "C:\\Pengguna\\COLORFUL\\Unduhan\\invoice_topup_saldo.txt"
            with open(invoice_path, "a") as invoice_file:
                invoice_file.write(invoice_content + "\n")
    ...
    if opsi == '1':
        ...
        if nohp_baru.isdigit() and len(nohp_baru) == 12:
            if nohp_baru in data["No_hp"]:
                ...
                print(F"PIN anda telah berhasil di update menjadi {nohp_baru}")
        UpdateTable()
        return
    else:
        print("Pilihan tidak valid!")
...
def urutan_akun(data):
    urutan_nama = sorted(zip(data["Nama_Panjang"], data["Username"], data["PIN"], data["No_HP"], data["Saldo_E Money"]), key=lambda x: x[0])
    return {
...
def buat_invoice_isisaldo(waktu, jumlah, penerima):
    try:
        idx_penerima = data['Username'].index(penerima)
        data['Saldo_E Money'][idx_penerima] += jumlah

        invoice = f"===== Detail Invoice =====\n"
        invoice += f"Waktu Transfer: {waktu}\n"
        invoice += f"Jumlah: Rp.{jumlah:,}\n"
        invoice += f"Dikirim ke: {penerima}\n"
        invoice += f"===========================\n"
        invoice += f"Terimakasih telah berkunjung di Hotel Jatra Balikpapan!\n"
        return invoice
    except ValueError:
        return "Username penerima tidak ditemukan di database."
